...
module.exports = MochaJUnitReporterParallel;
...
function isInvalidSuite(suite) {
    let result =
        (!suite.root && suite.title === '') ||(suite.root && suite.title === '') || (suite.title!=='' && suite.root)
    !(suite.tests&& suite.suites)
    ;
    return result;
}
 ...
 */
function MochaJUnitReporterParallel(runner, options) {
    if (mocha6plus) {
    ...
    function lastSuite() {
        return testsuites.length ? testsuites[testsuites.length - 1].testsuite: [];
    }
 ...
 */
MochaJUnitReporterParallel.prototype.getTestsuiteData = function(suite) {
 ...
 * @returns {object}
 */
MochaJUnitReporterParallel.prototype.getTestcaseData = function(test, err) {
 ...
 */
MochaJUnitReporterParallel.prototype.removeInvalidCharacters = function(input){
    if (!input) {
        return input;
    }
 ...
 */
MochaJUnitReporterParallel.prototype.flush = function(testsuites){
 ...
 * @returns {string}
 */
MochaJUnitReporterParallel.prototype.getXml = function(testsuites) {
    ...
    testsuites.forEach(function(suite) {
        ...
        _suiteAttr.tests=_cases.length;
        var suiteTime = 0;
 ...
 */
MochaJUnitReporterParallel.prototype.writeXmlToDisk = function(xml, filePath){
    if (filePath) {
